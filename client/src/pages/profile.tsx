...
export default function Profile() {
  ...
  const updateWhatsappToggleMutation = useMutation({
    mutationFn: async (autoWhatsappShare: boolean) => {
      if (!user?.id) throw new Error("No user found");
    ...
    onSuccess: async () => {
      toast({ 
        ...
        description: userForm.autoWhatsappShare ? "WhatsApp will open automatically" : "WhatsApp auto-open disabled" 
      });
    ...
    onError: (error) => {
      ...
      setUserForm(prev => ({ ...prev, autoWhatsappShare: user?.auto_whatsapp_share ?? true }));
      toast({
        title: "Error",
      ...
      <div className="screen-content space-y-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {/* WhatsApp Auto-Share Toggle - Only for garage admins */}
            {user?.role === 'garage_admin' && (
              <div className="flex items-center justify-between py-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <span className="block">Auto WhatsApp Share</span>
                    <span className="text-xs text-muted-foreground">Automatically open WhatsApp when generating invoices</span>
                  </div>
                </div>
                <Switch 
                  checked={userForm.autoWhatsappShare}
                  onCheckedChange={(checked) => {
                    setUserForm(prev => ({ ...prev, autoWhatsappShare: checked }));
                    updateWhatsappToggleMutation.mutate(checked);
                  }}
                  disabled={updateWhatsappToggleMutation.isPending}
                  data-testid="switch-auto-whatsapp"
                />
              </div>
            )}
